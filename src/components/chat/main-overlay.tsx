...
function BackBar({
  ...
  return (
    <div className="sticky top-0 z-10 flex items-center gap-3 md:gap-4 px-3 md:px-6 py-2.5 md:py-3 border-b border-[var(--border)] bg-[color-mix(in_oklch,var(--surface)_80%,transparent)] backdrop-blur">
      <button
        onClick={onClose}
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden bg-[var(--bg)]">
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="max-w-[880px] mx-auto px-4 py-6 md:px-10 md:py-10">
          {loading ? (
    ...
    <div className="flex-1 flex flex-col overflow-hidden bg-[var(--bg)]">
      ...
      {loading ? (
        ...
        <div className="flex flex-col md:flex-row flex-1 overflow-hidden">
          ...
          <div
            className="shrink-0 overflow-x-auto md:overflow-x-hidden md:overflow-y-auto border-b md:border-b-0 md:border-r border-[var(--border)] py-2 md:w-[220px] flex md:block"
          >
            {data.files.map((f) => (
              <button
                key={f.path}
                onClick={() => setSelectedFile(f.path)}
                className={`md:w-full flex items-center gap-2 px-3 py-1.5 text-left text-[12px] font-mono whitespace-nowrap md:whitespace-normal hover:bg-[var(--surface-raised)] transition-colors shrink-0 ${
                  f.path === selectedFile
