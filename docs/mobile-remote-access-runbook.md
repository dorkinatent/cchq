...
This is the manual checklist for verifying CCHQ works from a phone (LAN + Tailscale) and auto-starts on Mac boot. Use after merging the `mobile-remote-access` branch to main.

## One-time setup

1. **Merge the branch to main** (or cut a release) and check out on the Mac that will host CCHQ.
...
4. **Set a memorable Tailscale hostname** so CCHQ is reachable at `cchq.<tailnet>.ts.net` over Tailscale:
   ```bash
   ./scripts/setup-hostname.sh
   ```
   (Pass a different name as the first arg if `cchq` isn't what you want. Only touches Tailscale — leaves the Mac's own hostname alone.)
5. **Tailscale:** install on the Mac and on your iPhone. After step 4 the Tailscale hostname is already `cchq`. No per-app config needed.
6. **Front CCHQ with TLS on port 443** so the URL has no port number and HTTPS:
   ```bash
   ./scripts/setup-tailscale-serve.sh
   ```
   After this, CCHQ is reachable at `https://cchq.<tailnet>.ts.net/` from any device on your tailnet. Stop with `./scripts/stop-tailscale-serve.sh`.
...
- [ ] **Tailscale access:** from your phone (switch Wi-Fi off, cellular on), `https://cchq.<tailnet>.ts.net/` loads and the session list populates. (If you haven't run `setup-tailscale-serve.sh` yet, substitute `http://<mac>.<tailnet>.ts.net:3000/`.)
- [ ] **mDNS broadcast:** on a second Mac, `dns-sd -B _cchq._tcp` lists a `CCHQ` service.
...
- `~/Library/Logs/cchq/stdout.log` — Next.js stdout
- `~/Library/Logs/cchq/stderr.log` — Next.js stderr (and startup issues)
- `~/Library/Logs/cchq/supabase.log` — `supabase start` output from the launch script
...
| `curl localhost:3000` fails after reboot | Supabase not up, or `supabase` CLI not on PATH in launchd | Check `~/Library/Logs/cchq/supabase.log`; add `/opt/homebrew/bin` to the plist `EnvironmentVariables.PATH` if needed. |
...
| Multiple servers on port 3000 | `npm run dev` running alongside the agent | `launchctl unload ~/Library/LaunchAgents/app.cchq.plist` during development; re-load when done. |

## Development workflow (with the agent installed)

When you want to iterate on CCHQ, you have two options:
